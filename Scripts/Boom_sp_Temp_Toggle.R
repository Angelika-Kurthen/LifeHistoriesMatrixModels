#############################
# Boom Life Historysp temperature toggle
############################
...
for (te in 1:length(temp_seq)){
  ...
  size_means[te] <- sum(0.0071*(size_means[te])^2.832) # multiply relative size (which is also biologically plausible) by Benke et al 1999 Table 2 a and b params (M(mg) = aL^b) 
...
# no disturbances, no hydropeaking
discharge <- rep(0.1, time = length(temp$dts)) 
...
# create dataframe of winter post disturbance abundances under different temperature regimes
...
temp_dist_b <- bind_rows(winter, summer, .id = "season") # dataframe for winter and summer abundances
...
temp_size_b <- bind_rows(winter_size_means, summer_size_means, .id = "season")# dataframe for winter and summer individual biomass
...
deltatemp_b <- setNames(deltatemp_b, names(temp_dist_b[c(1,2,4)]))#need to rename columns to match 
temp_dist_b <- rbind(temp_dist_b[c(1,2,4)], deltatemp_b)# combine

# now we want to compile standing biomass for summer, winter, and difference between summer and winter 
deltasize_b <- setNames(deltasize_b, names(temp_size_b))# need to rename columns to match 
temp_size_b <- rbind(temp_size_b, deltasize_b)# combine
