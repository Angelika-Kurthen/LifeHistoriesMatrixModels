######################
# Multivoltinism Slow Life History
######################
library(pracma)

# load slow life history strategy model + bespoke functions
...
# mseq_d <- do.call(rbind, mget(seq_names))# rename columns
# colnames(mseq_d) <- c("Date", "Abund", "MeanTemp")
# 
# # subset one year 
# mseq_d <- mseq_d[which(mseq_d$Date >= "2035-01-01" & mseq_d$Date <= "2035-12-31"), ]
# mseq_d$Date <- as.Date(mseq_d$Date)
# mseq_d$Strategy <- rep("Slow", times = length(mseq_d$Date))
# 
...
for (i in 1:length(seqs)){
  temp$Temperature <- temp$Temperature + seqs[i]
  out <- Dmodel(discharge, temp, baselineK = 10000, disturbanceK = 40000, 
...
mseq_d <- do.call(rbind, mget(seq_names))
colnames(mseq_d) <- c("Date", "Abund", "MeanTemp")
mseq_d$Date <- as.Date(mseq_d$Date)
mseq_d$Strategy <- "Slow"

# subset one year 
mseq_d_1 <- mseq_d[which(mseq_d$Date >= "2035-01-01" & mseq_d$Date <= "2035-12-31"), ]
...
peak_counts_d_1 <- peak_results_d_1 %>%
  dplyr::count(Strategy, MeanTemp, name = "n_peaks")%>%
  complete(
    Strategy,
    MeanTemp = seqs
