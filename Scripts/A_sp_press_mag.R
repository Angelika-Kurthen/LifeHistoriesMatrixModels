...
for (i in 1:length(magnitudes)){
  out <- Amodel(discharge, temp, baselineK = 10000, disturbanceK = 40000, Qmin = 0.25, extinct = 50, iteration = 10, peaklist = magnitudes[i], peakeach = length(temp$Temperature))
...
a_magnitude_df <- as.data.frame(cbind(magnitudes, mag_response, rep("A", times = length(mag_response))))
