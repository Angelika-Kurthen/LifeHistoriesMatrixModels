#############################
# Slow Life History temperature toggle
############################
...
for (te in 1:length(temp_seq)){
  ...
  size_means[te] <- sum(0.0077*(size_means[te])^2.910) # multiply relative size (which is also biologically plausible) by Benke et al 1999 Table 2 a and b params (M(mg) = aL^b) 
...
# loop through temperature sequences, adding -10 to +10 to temperature regime
# extracting mean individual body size 6 timesteps post disturbance for each temperature regimesize_means <- vector()
for (te in 1:length(temp_seq)){
  temp$Temperature <- temp$Temperature + temp_seq[te]
  temp_regime[te] <- mean(temp$Temperature)
...
temp_dist_d <- bind_rows(winter, summer, .id = "season")# dataframe for winter and summer abundances
...
# now we want to calculate the difference between mean abundances, so wake summer - winter
deltatemp_d <- as.data.frame(cbind(rep(3, times = length(temp_regime)),temp_regime, summer[,3]-winter[,3]))# dataframe for winter and summer individual biomass
...
deltatemp_d <- setNames(deltatemp_d, names(temp_dist_d[c(1,2,4)])) # need to rename columns to match 
temp_dist_d <- rbind(temp_dist_d[c(1,2,4)], deltatemp_d) # combine
# now we want to compile standing biomass for summer, winter, and difference between summer and winter 
deltasize_d <- setNames(deltasize_d, names(temp_size_d))# need to rename columns to match 
temp_size_d <- rbind(temp_size_d, deltasize_d) # combine